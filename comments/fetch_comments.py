...
# Set up logger
logger = logging.getLogger('YouTubeCommentFetcher')
...
class YouTubeCommentFetcher:
    ...
    def fetch_video_metadata(self, video_id):
        ...
        try:
            ...
            logger.info(f"❌ Error fetching video metadata: {e}")
            return None
    ...
    def fetch_comments(self, video_id, max_results=100):
        ...
        if not video_metadata:
            logger.info("❌ Could not fetch video metadata")
            return 0
        ...
        try:
            ...
            try:
                ...
                conn.commit()
                logger.info(f"✅ Stored metadata for video: {video_metadata['title']}")
            ...
            try:
                ...
                logger.error(f"❌ Error fetching comments from YouTube API: {e}")
                return 0
        
            # Print total comments fetched
            total_fetched = len(comments_response.get('items', []))
            logger.info(f"📥 Total comments fetched from YouTube: {total_fetched}")
            ...
            for comment_thread in comments_response.get('items', []):
                ...
                try:
                    ...
                    logger.error(f"❌ Error inserting comment {comment_thread['id']}: {e}")
                    continue
        ...
        logger.info(f"✅ Successfully processed {total_fetched} comments:")
        logger.info(f"  - {inserted_count} new comments inserted")
        logger.info(f"  - {duplicate_count} duplicate comments skipped")
